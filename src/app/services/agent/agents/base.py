...
"""
    ...
    async def emit_thinking(self, message: str):
        ...
        try:
            cve = self._cve_id or "-"
            print(f"[THOUGHT][CVE={cve}][{self.name}] {message}", flush=True)
        except Exception:
            pass
    ...
    async def emit_llm_thought(self, thought: str, iteration: int):
        ...
        try:
            cve = self._cve_id or "-"
            print(f"[THOUGHT][CVE={cve}][{self.name}][iter={iteration}] {display_thought}", flush=True)
        except Exception:
            pass
        await self.emit_event(
            "llm_thought",
    ...
    def set_log_context(self, cve_id: str | None = None):
        """设置日志上下文（用于多线程日志分桶）。"""
        if cve_id:
            self._cve_id = str(cve_id).strip()
