...
3. 至少调用一个工具且必须包含 read_file；并至少再调用一个确认类工具（search_code / dataflow_analysis / vulnerability_validation / pattern_match / code_analysis）
...
7. Final Answer 前必须逐节点自检：对每个节点执行 `read_file(file_path=File, start_line=StartLine, end_line=EndLine)`，确认返回内容包含该节点 code；不满足则先修正行号或 code 再提交
8. `source_to_sink_path[].Desc` 必须写“节点在污点传播中的具体作用”，至少包含一个真实变量名和一个真实函数/操作名；禁止只写“用户可控数据在此进入/污点传播/危险操作触发”这类模板句
...
单轮输出必须二选一：
- 工具执行轮：`Thought + Action + Action Input`
- 结束提交轮：`Thought + Final Answer`
  ...
  "findings": [
    {
      ...
      "description": "...",
      "source_to_sink_path": [
        {"Type": "Source|Propagation|Sink", "File": "...", "StartLine": 0, "EndLine": 0, "Desc": "变量 `$userId` 通过 `GETPOST()` 接收外部输入并成为污点源", "code": "..."},
        {"Type": "Source|Propagation|Sink", "File": "...", "StartLine": 0, "EndLine": 0, "Desc": "`$userId` 经过 `implode()` 拼接进 `$sql`，污点继续传播到执行语句", "code": "..."}
      ],
      "suggestion": "...",
      "confidence": 0.0,
...
class AnalysisAgent(BaseAgent):
    ...
    def _ready_to_finalize(self) -> tuple[bool, str]:
        ...
        evidence_calls = sum(
            ...
            for name in ("search_code", "dataflow_analysis", "vulnerability_validation", "pattern_match", "code_analysis")
        )
        if total_actions < 1:
            return False, (
                "工具调用不足（至少需要1次）"
        ...
        if evidence_calls < 1:
            return False, (
                "缺少确认类工具调用（search_code/dataflow_analysis/vulnerability_validation/pattern_match/code_analysis）"
                f" [当前: total_actions={total_actions}, read_file={read_calls}, evidence={evidence_calls}]"
            )
        return True, ""
    ...
    async def run(self, input_data: Dict[str, Any]) -> AgentResult:
        ...
        semantic_snippets = config.get("patch_focus_semantic_snippets") or []
...
"""
        ...
        if isinstance(semantic_snippets, list) and semantic_snippets:
            initial_message += "## 补丁语义关注片段（old->new）\n"
            initial_message += "优先围绕这些语义修复点追踪 Source->Propagation->Sink，而不仅是变量名匹配。\n"
            for i, item in enumerate(semantic_snippets[:8], 1):
                if not isinstance(item, dict):
                    continue
                old = str(item.get("old") or "").strip()
                new = str(item.get("new") or "").strip()
                reason = str(item.get("reason") or "semantic_change").strip()
                if not old and not new:
                    continue
                initial_message += f"{i}. [{reason}] old: {old}\n   new: {new}\n"
            initial_message += "\n"
                    ...
                    if not ready:
                        ...
                        self._conversation_history.append({
                            ...
                            "content": (
                                ...
                                "请继续执行工具分析后再提交，最少要求：\n"
                                "1) 至少一次 read_file\n"
                                "2) 至少一次 search_code/dataflow_analysis/vulnerability_validation/pattern_match/code_analysis\n"
                                "3) 累计至少一次工具调用\n\n"
...
Final Answer:""",
            ...
            return AgentResult(
                ...
                data={
                    ...
                    "shared_tool_cache": self.export_shared_tool_cache(),
                    "tool_cache_digest": self.get_shared_tool_cache_digest(),
                    "steps": [
                        {
                            "thought": s.thought,
                            "action": s.action,
    ...
    def _create_analysis_handoff(self, findings: List[Dict[str, Any]]) -> TaskHandoff:
        ...
        context_data = {
            ...
            # 传递可复用工具证据，供 Verification Agent 优先复用
            "shared_tool_cache": self.export_shared_tool_cache(),
            "analysis_tool_evidence": self.get_shared_tool_cache_digest(),
        }
