...
## JSON 一次性提交硬约束
1. `Action Input` 必须始终是可被 `json.loads` 直接解析的单个 JSON 对象
2. 禁止使用 Markdown 代码块、注释、`...`、占位符文本
3. 当 `Action=finish` 时，`Action Input` 必须一次性给出完整结果，不要分轮补字段
4. 缺失字段也要明确给空值（`[]`、`""`、`null`）
...
class OrchestratorAgent(BaseAgent):
    ...
    def _emit_sub_agent_io(self, phase: str, agent_name: str, payload: Dict[str, Any]) -> None:
        """
        Emit sub-agent I/O payloads to terminal for observability.
        """
        def _compact(obj: Any, depth: int = 0) -> Any:
            if depth > 4:
                return "...(truncated)"
            if isinstance(obj, str):
                return obj if len(obj) <= 500 else obj[:500] + "...(truncated)"
            if isinstance(obj, list):
                items = [_compact(x, depth + 1) for x in obj[:20]]
                if len(obj) > 20:
            ...
            if isinstance(obj, dict):
                ...
                if len(obj) > 40:
                    out["__truncated__"] = f"{len(obj) - 40} more keys"
                return out
            return obj

        try:
            compact_payload = _compact(payload)
            text = json.dumps(compact_payload, ensure_ascii=False, indent=2, default=str)
        except Exception:
            text = str(payload)
        print(f"\n[SECAGENT][{phase}][{agent_name}]\n{text}\n", flush=True)

    @staticmethod
    def _compact_handoff(handoff: TaskHandoff, to_agent: str) -> TaskHandoff:
        """Reduce handoff size to avoid context bloat between agents."""
        compact_findings: List[Dict[str, Any]] = []
        for f in (handoff.key_findings or [])[:6]:
                    ...
                    compact_nodes.append(
                        {
                            ...
                            "File": str(n.get("File") or n.get("file_path") or "")[:200],
                            "StartLine": n.get("StartLine") or n.get("start_line") or n.get("line"),
                            "EndLine": n.get("EndLine") or n.get("end_line") or n.get("line"),
                            "Desc": str(n.get("Desc") or n.get("description") or "")[:160],
                            "code": str(n.get("code") or "")[:240],
                        }
                    )
            compact_findings.append({
                "title": str(f.get("title", ""))[:120],
                "vulnerability_type": str(f.get("vulnerability_type", "other"))[:60],
                "severity": f.get("severity", "medium"),
                "file_path": str(f.get("file_path", ""))[:200],
                "line_start": f.get("line_start"),
                "line_end": f.get("line_end"),
                "description": str(f.get("description", ""))[:220],
                "source_to_sink_path": compact_nodes,
        ...
        return TaskHandoff(
            ...
            to_agent=to_agent,
            summary=str(handoff.summary or "")[:300],
            work_completed=[str(x)[:120] for x in (handoff.work_completed or [])[:6]],
            key_findings=compact_findings,
            insights=[str(x)[:160] for x in (handoff.insights or [])[:8]],
            suggested_actions=[x for x in (handoff.suggested_actions or [])[:6] if isinstance(x, dict)],
            attention_points=[str(x)[:160] for x in (handoff.attention_points or [])[:10]],
            priority_areas=[str(x)[:200] for x in (handoff.priority_areas or [])[:10]],
...
请立即输出你的下一步决策。"""
                ...
                if step.action == "finish":
                    if self._requires_verification() and self._dispatched_tasks.get("verification", 0) == 0:
                        await self.emit_llm_decision("拒绝完成", "当前验证级别要求先调用 Verification Agent")
                        warning = (
                            "⚠️ 当前 verification_level 要求必须先调用 verification Agent，"
                            "请先执行 dispatch_agent 并设置 agent=verification，再进行 finish。"
                        )
                        step.observation = warning
                        await self.emit_llm_observation(warning)
                        self._conversation_history.append({
                            "role": "user",
...
请开始你的审计工作。首先思考应该如何开展，然后决定第一步做什么。"""
    ...
    def _requires_verification(self) -> bool:
        """当前配置是否要求必须调用 Verification Agent。"""
        cfg = self._runtime_context.get("config", {}) if isinstance(self._runtime_context, dict) else {}
        level = str(cfg.get("verification_level", "standard") or "standard").strip().lower()
        return level not in {"analysis_only", "none", "off", "disabled"}
    ...
    async def _dispatch_agent(self, params: Dict[str, Any]) -> str:
        ...
        if str(agent_name).lower() == "verification" and not self._requires_verification():
            return "当前 verification_level=analysis_only，已禁用 verification Agent 调度。请直接汇总 analysis 结果并 finish。"
...
"""
        ...
        try:
            ...
            if handoff:
                try:
                    handoff_size = len(json.dumps(handoff.to_dict(), ensure_ascii=False))
                except Exception:
                    handoff_size = -1
                logger.info(
                    f"[Orchestrator] Handoff to {agent_name}: findings={len(handoff.key_findings or [])}, size={handoff_size} chars"
                )
            ...
            self._emit_sub_agent_io(
                "INPUT",
                agent_name,
                {
                    "task": task,
                    "task_context": context,
                    "project_root": sub_input.get("project_root"),
                    "config": sub_input.get("config"),
                    "project_info": {
                    ...
                    "previous_results_summary": {
                        ...
                        "available_agent_results": list(self._agent_results.keys()),
                    },
                    "handoff": sub_input.get("handoff"),
                },
            )
            ...
            if result.success and result.data:
                ...
                if raw_findings:
                    ...
                    for new_f in valid_findings:
                        ...
                        if not normalized_new or not isinstance(normalized_new, dict):
                            continue

                        # Create fingerprint for strict deduplication
                        ...
                        # Check if exists (strict matching: same file + same line + same type)
                        found = False
                        for i, existing_f in enumerate(self._all_findings):
                            ...
                            # Strict match to avoid cross-line field pollution
                            ...
                            if same_file and same_line and same_type:
...
"""
    ...
    def _build_handoff_for_agent(
        ...
        if target_agent == "analysis" and "recon" in self._agent_handoffs:
            ...
            return self._compact_handoff(recon_handoff, target_agent)
        ...
        if target_agent == "verification" and "analysis" in self._agent_handoffs:
            ...
            return self._compact_handoff(merged_handoff, target_agent)
